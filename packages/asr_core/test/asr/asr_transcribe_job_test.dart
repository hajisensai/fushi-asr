...
void main() {
  ...
  test('对不齐的段只丢它自己：同批其余段照常落盘、任务照常完成', () async {
    ...
    // 被拒段连位置和原因一起落账，最终结果也带着它——不能「缺段却报正常完成」。
    final AsrTranscribeResult result =
        (events.last as AsrTranscribeFinishedEvent).result;
    expect(result.unalignedSegments, 1);
    expect(result.cueCount, 2);
    final List<AsrRejectedSegment> rejected =
        await AsrTranscribeJob.loadRejected(tmp);
    expect(rejected.single.reason, AsrAlignmentRejection.noAcousticEvidence);
    expect(rejected.single.text, 'あ。');
  });

  test('丢段计数跨暂停恢复不归零：被拒段落账、恢复后不重跑也不重复计', () async {
    // 回归：计数只活在内存里时，一暂停就归零；而被跳过的段恢复后不会重跑，
    // 于是「字幕缺了段」和「正常完成」同时成立。
    ...
    final AsrTranscribeJob first = AsrTranscribeJob(
      ...
      audioPaths: paths,
      modelId: _kModelId,
      pcm: _FakePcm(<String, int>{'a.mp3': 15000}),
      segmenter: seg,
      decoder: _FakeDecoder(),
      chunkSeconds: 5,
    ...
    expect(await AsrTranscribeJob.loadRejected(tmp), hasLength(1));
    ...
    final List<AsrTranscribeEvent> events = await second.run().toList();
    final AsrTranscribeResult result =
        (events.last as AsrTranscribeFinishedEvent).result;
    // 恢复后那一段不会重跑，账必须还在，且只记一次。
    expect(result.unalignedSegments, 1);
    // 内存计数也要从账本重建：只有落盘、进度里却归零，用户在跑的过程中仍然
    // 看不出字幕缺了段。
    for (final AsrTranscribeEvent e in events) {
      if (e is AsrTranscribeProgressEvent) {
        expect(e.progress.unalignedSegments, 1);
      }
    }
    expect(await AsrTranscribeJob.loadRejected(tmp), hasLength(1));
  });

  test('含推定端点的段随段落落盘，恢复后仍不算完整声学对齐', () async {
    final AsrTranscribeJob job = AsrTranscribeJob(
      jobDir: tmp,
      audioPaths: <String>['a.mp3'],
      modelId: _kModelId,
      ...
      alignSegment:
        ...
        estimatedBoundaries: 2,
      ),
    );
    final List<AsrTranscribeEvent> events = await job.run().toList();
    final AsrTranscribeResult result =
        (events.last as AsrTranscribeFinishedEvent).result;
    expect(result.estimatedBoundarySegments, 1);
    final List<AsrTranscribedSegment> stored =
        await AsrTranscribeJob.loadSegments(tmp);
    expect(stored.single.estimatedBoundaries, 2);
  });
