...
/// - 其余一律系统 CLI。
///
/// 本包只提供 CLI 后端（外部 `ffmpeg` / `ffprobe` 进程），目标宿主是桌面与服务端。
/// 移动端宿主（无系统 ffmpeg、iOS 禁 exec 子进程）应自己实现 [FfmpegBackend] 并经
/// `FfmpegAsrPcmSource(backend: ...)` 注入。
...
FfmpegBackend _selectBackend() => const CliFfmpegBackend();
