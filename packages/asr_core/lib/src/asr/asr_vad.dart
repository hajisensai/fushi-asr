...
/// - [EnergyVadScorer]：纯 Dart 自适应能量门限，零模型调用。**这是一个带前提的
///   优化，不是默认值**——前提是「语音与背景在能量上双模态可分」。有声书是干净
///   的朗读录音，句间静默与人声的能量差 30 dB 以上，不需要神经网络来分辨；
///   2026-09-05 Windows 真机分阶段计时里 silero 每窗口约 2.8 ms（78 s 音频 6.8 s，
///   占整条流水线七成），能量打分器把这一项降到可忽略。
///
///   前提不成立时它不会「差一点」，而是自信地全判成语音：2026-09-10 用一集
///   Re:Zero（内封英文字幕作真值）实测，能量门限把 89.3% 的片长（219 段 /
///   1428.1 s）送进 ASR，其中 34.6%（493.7 s）落在任何对白之外，31 段（14.2%）
///   整段没有一句对白——每段都是一次幻听机会。选哪一条由 `AsrAudioProfile`
///   显式声明，那个枚举**故意没有默认值**。
/// - [SileroVadScorer]：k2-fsa 导出的 silero-vad v4（仅 16 kHz 分支，IO 见
///   `asr_types.dart`），每窗口一次前向，携带 LSTM 状态 `h`/`c`（`[2,1,64]`，换文件
///   清零）。混音素材（动画/影视/任何带 BGM 的音源）只能用它。
///
