...
/// 又都有发射时间（`AlignCue.tokenTiming`），于是可以：
///
...
class CueSentenceResegmenter {
  ...
  CueResegmentResult resegment({
    ...
    final List<AlignCue> outCues = <AlignCue>[];
  ...
  static bool _eligible(AlignCue cue, CueMatch m, int sectionCount) {
    final AsrCueTokenTiming? timing = cue.tokenTiming;
    return m.matched &&
  ...
  _RunOutput _resegmentRun({
    ...
    final AlignCue proto = cues[from];
    final List<AlignCue> outCues = <AlignCue>[];
    final List<CueMatch> outMatches = <CueMatch>[];
    for (int k = 0; k < n; k++) {
      final _Piece p = pieces[k];
      final AlignCue cue = AlignCue()
