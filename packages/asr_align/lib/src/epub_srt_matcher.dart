...
class EpubSrtMatcher {
  ...
  static Future<MatchResult> matchInIsolate({
    ...
    final _MatchRequest req = _MatchRequest(
      sections: sections,
      cueTexts: <String>[for (final AlignCue c in cues) c.text],
      cueIndexes: <int>[for (final AlignCue c in cues) c.sentenceIndex],
  ...
  static ProbeResult probe({
    ...
    final List<String> normCueTexts = <String>[
      for (final AlignCue c in cues) AudioTextNormalizer.normalize(c.text),
    ];
...
MatchResult _matchEntrypoint(_MatchRequest req) {
  final List<AlignCue> cues = _rebuildCues(req.cueTexts, req.cueIndexes);
  if (cues.isEmpty) {
    return const MatchResult(
      matches: <CueMatch>[],
      totalCues: 0,
...
List<AlignCue> _rebuildCues(List<String> texts, List<int> indexes) {
  return <AlignCue>[
