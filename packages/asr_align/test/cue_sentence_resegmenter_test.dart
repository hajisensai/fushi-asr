...
AlignCue _cue(
  ...
  return AlignCue()
    ...
    ..endMs = end
    ..audioFileIndex = 0
    ..tokenTiming = AsrCueTokenTiming(tokens: toks, offsetsMs: ts);
}
...
void main() {
  group('CueSentenceResegmenter', () {
    ...
    test('一条盖两句的 cue 在正文句号处拆开，新边界取下一句首 token − leadIn', () {
      ...
      final List<AlignCue> cues = <AlignCue>[
    ...
    test('串内既有句号新增边界、又有词中边界抹掉：保留的原边界时间不变', () {
      ...
      expect(out.stats.boundariesRemoved, 1);
      expect(out.stats.boundariesAdded, 0);
      expect(out.cues.map((AlignCue c) => c.text), <String>[
        'たとえば夢見る時がある',
        '転入生がやってくる',
      ]);
    ...
    test('模糊命中（听写差）走编辑距离映射，句号仍切在正确位置', () {
      ...
      final CueResegmentResult out = _r.resegment(
        sections: sections,
        cues: <AlignCue>[_cue(0, text, start: 0, end: 5000, times: times)],
    ...
    test('未命中 / 没有 token 时间 / 区间不相接的 cue 原样透传', () {
      final AlignCue noTiming = AlignCue()
        ..bookKey = 'b'
        ..chapterHref = 'ch0.xhtml'
        ..sentenceIndex = 0
        ..textFragmentId = ''
        ...
        ..endMs = 3000
        ..audioFileIndex = 0;
      final AlignCue miss = _cue(1, 'ざつおん', start: 3000, end: 4000);
      final AlignCue apart = _cue(2, 'その子は', start: 4000, end: 5000);
      final CueResegmentResult out = _r.resegment(
        sections: sections,
        cues: <AlignCue>[noTiming, miss, apart],
    ...
    test('matches 与 cues 数量不符时原样返回', () {
      final CueResegmentResult out = _r.resegment(
        sections: sections,
        cues: <AlignCue>[_cue(0, 'たとえば', start: 0, end: 1)],
        result: _result(<CueMatch>[]),
      );
      expect(out.cues, hasLength(1));
