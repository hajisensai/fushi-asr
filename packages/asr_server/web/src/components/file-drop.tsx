...
const fileKinds = {
  epub: { Icon: BookOpenIcon, id: 'epub', accept: '.epub,application/epub+zip' },
  audio: { Icon: FileAudioIcon, id: 'file', accept: 'audio/*,video/*,.m4b,.mka,.opus,.flac' },
  subtitle: { Icon: CaptionsIcon, id: 'subtitle', accept: '.srt,.vtt,application/x-subrip,text/vtt' },
} as const;
export function FileDrop({ kind, file, disabled, onChange }: Props) {
  ...
  const { Icon, id, accept: extensions } = fileKinds[kind];
  const title = t(`file.${kind}.title`), prompt = t(`file.${kind}.prompt`), note = t(`file.${kind}.note`);
    ...
    <div className="relative min-w-0" onDragEnter={e => { e.preventDefault(); if (!disabled) { depth.current++; setDragging(true); } }}
      ...
      <Button ref={trigger} id={`${kind}-drop`} data-testid={`${kind}-drop`} variant="outline" disabled={disabled}
        ...
        <span className="flex min-w-0 flex-col items-center gap-3 w-full">
          ...
          <span className="max-w-full whitespace-normal break-all">{dragging && !disabled ? t('file.release') : file ? file.name : prompt}</span>
          <span>{file ? t('file.replace', { size: (file.size / 1024 / 1024).toFixed(1) }) : t('file.pick')}</span>
        </span>
      </Button>
      {file ? <div className="absolute top-2 right-2"><Button variant="ghost" size="icon-sm" disabled={disabled} aria-label={t('file.remove', { title })} onClick={() => { onChange(null); setError(null); if (input.current) input.current.value = ''; trigger.current?.focus(); }}><XIcon data-icon="inline-start" /></Button></div> : null}
